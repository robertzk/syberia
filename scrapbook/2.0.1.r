...
(list(
  ...
  data = list(
   ...
   #,"Computing income ratio"         = c( multi_column_transformation(function(income, median_income) 13 * income / median_income), c('income', 'median_income'), 'income_ratio')
    ...
    ,"Setup validation data"          = list( function(dataframe) eval(substitute({ dataframe <- dataframe[1:(0.8*nrow(dataframe)), ] }), envir = parent.frame()))
    ,"Sure independence screening"    = list( function(dataframe) eval(substitute({
     do_not_discretize <-
